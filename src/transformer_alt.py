...
class Transformer(nn.Module):
    def __init__(
        ...
        for i in range(tf_blocks):
            embed_dim_out = int(embed_dim * embed_dim_ratio**(i + 1))  
                  
            embed_dim_in = (embed_dim_in // heads)*heads  # make sure embed_dim_in is divisible by heads
            embed_dim_out = (embed_dim_out // heads)*heads  # make sure embed_dim_out is divisible by heads
            block = TransformerBlock(vocab_size, max_seq_len, heads, embed_dim_in, embed_dim_out, 4*embed_dim_in, dropout, start_token_id)
            layer_list.append(block)

            embed_dim_in = embed_dim_out
        ...
        if not use_weight_tying:
            self.word_unembed = nn.Linear(embed_dim_out, vocab_size, bias=False).weight
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.head_dim = embed_dim_in // heads

        self.dol1 = nn.Dropout(dropout)
        self.dol2 = nn.Dropout(dropout)
        self.do_attn = nn.Dropout(dropout)

        self.ln1 = nn.LayerNorm(embed_dim_in, eps=1e-6)
        self.ln2 = nn.LayerNorm(embed_dim_out, eps=1e-6)

        self.KQV = nn.Linear(embed_dim_in, embed_dim_in * 3, bias=False)
        self.WO = nn.Linear(embed_dim_in, embed_dim_in, bias=False)

        self.layer_up = nn.Linear(embed_dim_in, ff_dim, bias=True)
        self.layer_down = nn.Linear(ff_dim, embed_dim_out, bias=True)
    ...
    def attention(self, x_embeds, mask):
        ...
        out = torch.reshape(out, [batch, seq, self.embed_dim_in])
    ...
    def ffnn(self, x_embeds):
        ...
        if self.embed_dim_in < self.embed_dim_out:
            x_pad = F.pad(x_embeds, pad=(0, self.embed_dim_out - self.embed_dim_in), value=0)
        else:
            x_pad = x_embeds[:, :, :self.embed_dim_out]

        out = self.ln2(out + x_pad)
